...
// import subworkflows
include {tm10_preprocessing} from './testing_modules/workflows/TM10.preprocessing.nf'
include {tm10} from './testing_modules/workflows/TM10.nf'
...
workflow {
    ...
    tm10_preprocessing(input_files, krakenDB, bowtie_dir)

    if ( params.unmix_myco == "yes" ) {
        clockwork_seqs = tm10_preprocessing.out.decontam_seqs
        clockwork_json = tm10_preprocessing.out.decontam_json
    }

    if ( params.unmix_myco == "no" ) {
        clockwork_seqs = tm10_preprocessing.out.uncontam_seqs
        clockwork_json = tm10_preprocessing.out.uncontam_json
    }

    tm10(clockwork_seqs.join(clockwork_json, by: 0))
}
